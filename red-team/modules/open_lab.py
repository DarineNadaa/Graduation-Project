...
reachability check, not the real attack — real scoring for freestyle
sessions would need a bridge that polls ZAP's alerts queue separately
(not wired up yet on this branch).
"""
...
class OpenLabModule(BaseModule):
    ...
    def execute(self, target: TargetConfig, opts: dict, log_fn=None) -> AttackResult:
        ...
        if step.success:
            summary = (
                "Open lab session ready — target reachable. Use AttackBox + ZAP freely."
            )
        else:
