...
from .dependencies import get_event_emitter, get_hive_client, get_sandbox_connector, get_enrichment_service, get_settings, require_room
...
def api_raise_alert(
    ...
    room_id: str = Depends(require_room),
...
def api_investigate_alert(
    ...
    """
    try:
        return investigate_alert(body=body, emitter=emitter, room_id=room_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc))
...
def api_deny_alert(
    ...
    """
    try:
        return deny_alert(body=body, emitter=emitter, room_id=room_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc))
...
def api_confirm_incident(
    ...
    """
    try:
        return confirm_incident(body=body, emitter=emitter, hive=hive, room_id=room_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc))
...
def api_initiate_containment(
    ...
    """
    try:
        return initiate_containment(body=body, emitter=emitter, sandbox=sandbox, room_id=room_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc))
...
def api_complete_containment(
    ...
    """
    try:
        return complete_containment(body=body, emitter=emitter, room_id=room_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(exc))
