...
class AttackSession:
    ...
    def __init__(
        ...
        # Room-scoped incident, if the operator has joined one -- see
        # backend/identity.py / attense-app's GET /api/auth/me. None falls
        # back to the env var INCIDENT_ID in engine.run_module().
    ...
    def _cmd_execute(self, _args) -> None:
        ...
        try:
            self.result = engine.run_module(
                ...
                actor_id=self.actor_id, incident_id=self.incident_id,
            )
            self.timer.stop()
