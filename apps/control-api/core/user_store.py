...
def _load_users() -> list[dict]:
    ...
    for user in users:
        ...
        user.setdefault("active_room_id", None)
    return users
...
def set_active_room(user_id: str, room_id: str) -> dict:
    """Record the most recently joined room for a red_team user. Used as the
    tie-break when a user belongs to more than one room — last join wins."""
    users = _load_users()
    for user in users:
        if user["id"] == user_id:
